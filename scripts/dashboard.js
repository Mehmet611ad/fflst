...
const Dashboard = {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    if (response.success) {
                        ...
                        Auth.showSuccess(`"${file.name}" başarıyla yüklendi!`);
                    }
                } catch (error) {
                    Auth.showError('upload-error', `"${file.name}" yüklenemedi`);
                }
            }
        } finally {
        ...
        if (uploadArea) {
            ...
            if (uploadContent) {
                uploadContent.innerHTML = `
                    ...
                    <p>veya</p>
                    <button id="browse-files-btn" class="btn secondary-btn">Dosya Seçin</button>
    ...
    async downloadFile(fileName) {
        try {
            ...
            if (response.success) {
                Auth.showSuccess(`"${fileName}" indiriliyor...`);
                ...
                setTimeout(() => {
                    ...
                    Auth.showSuccess(`"${fileName}" başarıyla indirildi!`);
                }, 100);
            } else {
                Auth.showError('download-error', response.message || 'Dosya indirilemedi');
            }
        } catch (error) {
            Auth.showError('download-error', 'Dosya indirilemedi');
        }
    },
    ...
    showFileDetails(fileName) {
        ...
        document.getElementById('detail-size').textContent = 'Bilinmiyor'; // Would be retrieved from file metadata
        document.getElementById('detail-date').textContent = new Date().toLocaleString('tr-TR');
    ...
    async deleteFile(fileName) {
        try {
            ...
            if (response.success) {
                // Refresh file list
                await this.loadUserFiles();
                Auth.showSuccess('Dosya başarıyla silindi!');
            } else {
                Auth.showError('delete-error', response.message || 'Dosya silinemedi');
            }
        } catch (error) {
            Auth.showError('delete-error', 'Dosya silinemedi');
        }
    },
